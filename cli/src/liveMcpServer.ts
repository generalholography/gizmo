...
import { buildWorldEditorPrompt, createAutomationMcpServer } from '@gizmo3d/mcp';
...
export async function createLiveProxyMcpServer(serverUrl: string, token?: string) {
  ...
  const server = await createAutomationMcpServer(session, {
    ...
    buildPromptText: (info, goal) => buildWorldEditorPrompt({ ...info, serverUrl: info.serverUrl || serverUrl }, goal),
  });
